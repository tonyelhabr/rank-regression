...
between paired samples using the ranking of the variables (not their values).
...
with those of statistical theory.
...
(__NOTE:__ I explicitly list the variable names that I use in the code implementation to help
with orientation.)
...
as "sufficiently" __monotonic__. (This `mntc_tier2` variable can be interpreted as the heuristic.)
...
range, the values should be interpreted as ranks (the "actual" ranks).
...
This result is generally true for the simulation performed.
(For example, if I chose `n = 5` initially, then there would be `6` unique values of each metric.)

Now, to understand how the _Pearson's rho_ estimate and p-values correspond to my heuristic,
