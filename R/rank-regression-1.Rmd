...
__NOTE:__ This write-up is split into two parts. This first part constitutes the "meat"
...
readr::write_csv(summ_mntc_byn, file.path("data", "summ_mntc_byn.csv"))
```
```{r summ_mntc_byn_import, include = TRUE, echo = FALSE}
summ_mntc_byn <- readr::read_csv(file.path("data", "summ_mntc_byn.csv"))
```
...
```{r viz_mntc_tier2_byn, include = TRUE, echo = FALSE, fig.height = 6, fig.width = 8}
lab_title_mntc_tier2_byn <-
  gsub("^.*,", "Distribution of P-Values,", lab_title_mntc_distinv_byn)
lab_subtitle_mntc_tier2_byn <-
  "By Custom Heuristic Classification"
lab_caption_mntc_tier2_byn <-
  paste0(
    "Expectation: Green more dense near 0 (right); red more dense near 1 (left).\n",
...
viz_mntc_tier2_byn <-
  ...
  ggplot(aes(x = p.value, fill = mntc_tier2)) +
  geom_histogram(bins = 20) +
